...
class UltimateSQLFiller():
    ...
    _______
    Methodes:
        - data_base_connection: connects to data base
        - excute_sql_line
        - fill_species_assemblies_tables
        - browse_data fills the data in the connected database 
        - fill_genomicannotation_table
        - fill_proteinset_table
        - fill_blastdb_table
        - update_Assembly_table
        - fill_featureannotation_table
        - fill_noncodingrna_table
        - spiecies_extractorV3
    ...
    def data_base_connection(self):
        ...
        try:
            self.conn = connect(user=DATABASES['default']['USER'],
                                ...
                                host=DATABASES['default']['PASSWORD'],
        ...
        for line in self.file_lines[::-1]:
            ...
            if "raw_dir" in my_file[-4]:  # raw_dir directory
                ...
                if "nonMammalianVertebratesRefSeq" in my_file[-5]:  
                    if True:  # WAS: if "active_June21" in my_file[-3]: 
                    ...
                    if True:  # WAS: if "unique_GCF_June21" in my_file[-3]: 
                        if "genomic.fna" in my_file[-1]:
                            my_values = [specie_GCF[specie_GCF.index('_')+1:specie_GCF.index('.')],
                                         specie_GCF[specie_GCF.index('.')+1:],
                                         line[:-12],
